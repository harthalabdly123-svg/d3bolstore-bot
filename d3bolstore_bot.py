        ...
        for change in changes:
            if change.type.name == "ADDED":
                ...
                if user.get("status") == "pending":
                    threading.Thread(
                        target=lambda doc_id=change.document.id, u=user: send_user_notification(bot_app, doc_id, u),
                        daemon=True
                    ).start()
    db.collection("users").where("status", "==", "pending").on_snapshot(on_snapshot)

def send_user_notification(bot_app, user_id, user):
    import asyncio
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    loop.run_until_complete(_notify_new_user(bot_app, user_id, user))
    loop.close()

async def _notify_new_user(bot_app, user_id, user):
    text = (
        f"👤 *تسجيل حساب جديد!*\n\n"
        f"الاسم: {user.get('name','—')}\n"
        f"البريد: {user.get('email','—')}\n"
        f"تيليجرام: {user.get('telegram','—')}\n"
        f"التاريخ: {user.get('createdAt','—')[:10]}\n"
    )
    keyboard = [[
...
async def button_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "approve_user":
        db.collection("users").document(order_id).update({"status": "active"})
        user_doc = db.collection("users").document(order_id).get()
        if user_doc.exists:
            user_data = user_doc.to_dict()
            tg = user_data.get("telegram","").replace("@","")
            if tg and tg != "غير محدد":
                try:
                    await ctx.bot.send_message(
                        chat_id=f"@{tg}",
                        text="✅ *تم تفعيل حسابك في دعبول ستور!*\n\nيمكنك الآن تسجيل الدخول والاشتراك 🚀",
                        parse_mode="Markdown"
                    )
                except:
                    pass
        await query.edit_message_text(query.message.text + "\n\n✅ *تم تفعيل الحساب*", parse_mode="Markdown")
        return

    elif action == "reject_user":
        db.collection("users").document(order_id).update({"status": "rejected"})
        user_doc = db.collection("users").document(order_id).get()
        if user_doc.exists:
            user_data = user_doc.to_dict()
            tg = user_data.get("telegram","").replace("@","")
            if tg and tg != "غير محدد":
                try:
                    await ctx.bot.send_message(
                        chat_id=f"@{tg}",
                        text="❌ عذراً، تم رفض طلب تسجيل حسابك. للاستفسار تواصل مع الأدمن.",
                    )
                except:
                    pass
        await query.edit_message_text(query.message.text + "\n\n❌ *تم رفض الحساب*", parse_mode="Markdown")
        return
...
# ─── استقبال صورة الحوالة من المستخدم ───────────────────────
async def receive_receipt(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    
    # جيب آخر طلب للمستخدم
    try:
        users_q = db.collection("users").where("tgId", "==", str(user_id)).stream()
        uid = None
        for u in users_q:
            uid = u.to_dict().get("uid")
            break
        
        # ابحث عن طلب بانتظار التفعيل
        orders_q = None
        if uid:
            orders_q = db.collection("orders").where("uid", "==", uid).where("status", "==", "بانتظار التفعيل").stream()
        ...
        caption = (
            ...
            f"🆔 تيليجرام: @{update.effective_user.username or 'غير محدد'}\n"
        )
        ...
        await update.message.reply_text(
            "✅ تم إرسال صورة الحوالة!\n"
            "سيتم مراجعتها وتفعيل اشتراكك قريباً 🚀"
        )
    except Exception as e:
        await update.message.reply_text("✅ وصلت الصورة! سيتواصل معك الأدمن قريباً.")
...
def main():
    ...
    # Start Firestore watchers in background
    watcher_thread = threading.Thread(target=watch_orders, args=(app,), daemon=True)
    watcher_thread.start()
    users_thread = threading.Thread(target=watch_users, args=(app,), daemon=True)
    users_thread.start()
